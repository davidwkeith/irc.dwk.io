...
The base layout imports `global.css` (via `../styles/global.css`). After the design interview, `tokens.css` is generated by `scripts/design.ts` into `src/design/`. To apply the generated tokens, `global.css` should import or reference `tokens.css` (e.g., `@import '../design/tokens.css'`), so changes take effect immediately.
